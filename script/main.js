//import {getRegion, listLocation} from './modules.js';
...
function fetchPokemonImg(url){
    ...
    .then(response => response.json())
    .then(function(data) {
        const html = `<h1 class="pokemon-name">${data.species.name[0].toUpperCase() + data.species.name.slice(1)}</h1><img class="pokemon-image" src="${data.sprites.front_default}"></img>
        <img class="captureBtn" src="./img/pokeball.gif" alt="pokeball">`
        document.querySelector('.encounter-con').innerHTML = html;
        document.querySelector('.status').innerHTML = `${generateStat(data.stats)}`;
...
function display(arr, target)
        {
         target.innerHTML = arr.map(r => `<option value="${r.url}">${r.name[0].toUpperCase() + r.name.slice(1)}</option>`)
        }
